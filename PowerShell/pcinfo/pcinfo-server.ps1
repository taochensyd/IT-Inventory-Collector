$scriptBlock = {
    ...
    # For demonstration purposes, I'm assuming the machine running the Invoke-Command has an API listening on 192.168.1.XXX (replace XXX with actual IP)
    Invoke-RestMethod -Method Post -Uri "http://192.168.1.XXX:9000/api/v1/pcinfo" -Body $jsonData -ContentType "application/json"
}

# Request the credentials for the remote machine
$credentials = Get-Credential

# Use Invoke-Command to execute the script block on the remote machine
Invoke-Command -ComputerName 192.168.1.125 -ScriptBlock $scriptBlock -Credential $credentials
